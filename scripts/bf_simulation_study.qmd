...
sim_analyse <- function(condition, dat, fixed_objects = NULL) {
  ...
  ret_analyse <- data.frame(
    # Beta
    bf_beta = Rmpfr::asNumeric(mat_comp_beta[[1]]),
    log_bf_beta = Rmpfr::asNumeric(mat_comp_beta[[2]]),
    post_in_null_beta = mat_comp_beta[[3]],
    # Rho
    bf_rho = Rmpfr::asNumeric(mat_comp_rho[[1]]),
    log_bf_rho = Rmpfr::asNumeric(mat_comp_rho[[2]]),
...
sim_summarise <- function(condition, results, fixed_objects = NULL) {
  ...
  bf_mean_beta <- median(results$bf_beta)
  bf_sd_beta <- mad(results$bf_beta)
  bf_mean_rho <- median(results$bf_rho)
  bf_sd_rho <- mad(results$bf_rho)
  
  # Return all results
  ret_summarise <- data.frame(bf_mean_beta = bf_mean_beta,
...
saveRDS(res_df, here::here("output", "bf_sim_res_df"))
head(res_df)
```
