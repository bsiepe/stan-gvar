...
gvar_mod2 <- l_changed_graphs_0305[["graph5"]][["const0.1"]]
...
BS: Please add short descriptions to what you are doing. If not, this will get out of hand very soon. 

So I think now this takes the sum of differences per matrix. This is a bit weird, because large positive and negative differences could cancel out in a matrix, right?
...
compare_mat <-
  function(mat1,
    ...
    if (bootstrap_samples > 0) {
      rows1 <-
        sample(1:nrow(mat1),
               size = bootstrap_samples * ncol(mat1),
               replace = TRUE)
      rows2 <-
        sample(1:nrow(mat2),
               size = bootstrap_samples * ncol(mat1),
      ...
      # BS: This bootstraps columns (i.e. variables), right?
      # But it makes more sense to keep the full matrices intact and bootstrap rows (i.e. samples)
      col1 <-
        purrr::map2(
          .x = rows1,
          .y = cols,
    ...
    if (isTRUE(plot)) {
      ...
      df_samples <- data.frame(posterior = diff_post,
                               prior = diff_prior) %>% 
        tidyr::pivot_longer(cols = c(posterior, prior),
      ...
      plot <- df_samples %>%
        ...
        ggplot2::geom_density(aes(value, fill = distribution),
                              alpha = .5) +
        geom_vline(xintercept = null_ci$CI_low) +
        geom_vline(xintercept = null_ci$CI_high) +
