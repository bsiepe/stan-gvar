...
```{r message=FALSE}
net_1a_fit <- fit_gVAR_stan(data = data_1a, priors = priors, iter_sampling = 2000)
net_1b_fit <- fit_gVAR_stan(data = data_1b, priors = priors, iter_sampling = 2000)
net_2_fit <- fit_gVAR_stan(data = data_2, priors = priors, iter_sampling = 2000)
net_3_fit <- fit_gVAR_stan(data = data_3, priors = priors, iter_sampling = 2000)
```
...
```{r}
a <- beta_post_1a - beta_post_1b %>% as.data.frame()
b <- a^2
c <- 
```
...
```{r}
compare_mat <- function(mat1, mat2) {
  diff_post <- mat1 - mat2 %>% as.data.frame()
  
  mean <- diff_post %>% apply(2, mean) 
  
  diff_post <- diff_post %>% 
    apply(2, scale) %>%
    apply(1, function(x){x+mean}) %>%
    apply(2, function(x) {
      x ^ 2
    }) %>%
    apply(., 1, sum)
  ...
  # compute bayesfactor
  bf <- bayestestR::bf_parameters(posterior = diff_post, prior = diff_prior, direction = ">")
  print(bf)
}
```

```{r}
compare_mat(beta_post_1a, beta_post_1b)
compare_mat(beta_post_1a, beta_post_2)
compare_mat(beta_post_1a, beta_post_3)
```
```{r}
