...
compare_mat_Beta <-
  function(mat1,
           ...
           BF_01_N <- mean(exp(
             dnorm(diff_null, mean_post_u, sd_post_u, log = TRUE) -
             dnorm(diff_null, mean_prior_u, sd_prior_u, log = TRUE)))

           
           mean_post <- apply(diff_post_mat, 2, mean)
           sd_post <- apply(diff_post_mat, 2, sd)
           Sigma_post <- diag(sd_post)
                      
           mean_prior <- apply(diff_prior_mat, 2, mean)
           sd_prior <- apply(diff_prior_mat, 2, sd)
           Sigma_prior <- diag(sd_prior)
           
           dens <- sapply(1:nrow(diff_null_mat), function(i) {
               mvnfast::dmvn(diff_null_mat[i,], mean_post, Sigma_post, log = TRUE) -
               mvnfast::dmvn(diff_null_mat[i,], mean_prior, Sigma_prior, log = TRUE)
           }, simplify = TRUE)
           print(is.na(dens) %>% as.numeric() %>% mean())
           
           BF_01_MVN <- mean(exp(dens), na.rm = TRUE)
           ...
           # clean up
           rm(mat1, mat2, diff_post, diff_post_mat, diff_prior, diff_prior_mat, diff_null)
           }
```
