...
sim_summarise <- function(condition, results, fixed_objects = NULL) {
  ...
  for(i in seq_along(results)){
    ...
    pairwise_comp[[i]]$mean_pcor_diff_lkj_iw <- summary_iw[[i]]$pcor_mean - summary_lkj[[i]]$pcor_mea
  ...
  bias_beta_bggm <- lapply(comp_bggm, function(x){mat_bias(x$beta_diff)})
  bias_beta_iw <- lapply(comp_iw, function(x){mat_bias(x$beta_diff)})
  bias_beta_lkj <- lapply(comp_lkj, function(x){mat_bias(x$beta_diff)})
  mse_beta_bggm <- lapply(comp_bggm, function(x){mat_mse(x$beta_diff)})
  mse_beta_iw <- lapply(comp_iw, function(x){mat_mse(x$beta_diff)})
  mse_beta_lkj <- lapply(comp_lkj, function(x){mat_mse(x$beta_diff)})
  
  
  bias_pcor_bggm <- lapply(comp_bggm, function(x){mat_bias(x$rho_diff)})
  bias_pcor_iw <- lapply(comp_iw, function(x){mat_bias(x$rho_diff)})
  bias_pcor_lkj <- lapply(comp_lkj, function(x){mat_bias(x$rho_diff)})
  mse_pcor_bggm <- lapply(comp_bggm, function(x){mat_mse(x$rho_diff)})
  mse_pcor_iw <- lapply(comp_iw, function(x){mat_mse(x$rho_diff)})
  mse_pcor_lkj <- lapply(comp_lkj, function(x){mat_mse(x$rho_diff)})
  ...
  ret_summarise <- c(bias_beta_bggm = bias_beta_bggm,
                     ...
                     pairwise_mean_abs_diff = unlist(pairwise_mean_abs_diff))
...
sim_results <- SimDesign::runSimulation(design = df_design, 
                                    ...
                                    parallel = TRUE,
                                    packages = c("tidyverse", "BGGM", "graphicalVAR", "rstan", "here")
                                    # debug = "summarise"
                                    # save_results = TRUE,
                                    # filename = here("output/implementation-sim-results.rds"),
                                    # save_seeds = TRUE
                                    )
...
```
Extract some numerical results for plotting
```{r}
# lapply over the list and extract everything that does not start with "comp"
sim_sum_num <- lapply(sim_sum, function(x){
  x[!grepl("^comp", names(x))]
})
