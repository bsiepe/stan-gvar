...
sim_summarise <- function(condition, results, fixed_objects = NULL) {
  ...
  compute_mean_ess <- function(fit, iter, burnin) {
    mcmc_samples <- coda::as.mcmc(t(matrix(fit, nvar * nvar, iter - burnin)))
    mean(coda::effectiveSize(mcmc_samples))
  }

  # Define a function to separate the chains again for Stan estimation 
  split_posterior_samples <- function(posterior_samples_array, num_chains = 4,     num_iterations_per_chain = 500) {
  array_dimensions <- dim(posterior_samples_array)
  
  # Calculate the total number of iterations
  total_iterations <- array_dimensions[3]
  
  # Check if the total number of iterations is divisible by the number of chains
  if (total_iterations %% num_chains != 0) {
    stop("Total iterations is not divisible by the number of chains.")
  }
  
  # Calculate the number of iterations per block
  iterations_per_block <- total_iterations / num_chains
  
  # Reshape the array to combine the chains and iterations
  reshaped_array <- array(posterior_samples_array, c(array_dimensions[1], array_dimensions[2], total_iterations))
  
  # Use matrix indexing to create a list of split arrays
  split_arrays_list <- lapply(1:num_chains, function(chain) {
    start_index <- ((chain - 1) * iterations_per_block) + 1
    end_index <- chain * iterations_per_block
    return(reshaped_array[chain, , start_index:end_index, drop = FALSE])
  })
  
  return(split_arrays_list)
}
  
  test_split <- split_posterior_samples(results[[1]]$fit_stan_lkj$beta)
  
  # Process results for bbgm
  ess_beta_bggm <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_bggm$fit$beta, bggm_iter, burnin)))
  ess_pcor_bggm <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_bggm$fit$pcors, bggm_iter, burnin)))
  
  # Process results for lkj
  n_chains <- 4
  ess_beta_lkj <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_stan_lkj$beta, stan_iter*n_chains, 0)))
  ess_pcor_lkj <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_stan_lkj$rho, stan_iter*n_chains, 0)))
  
  # Process results for iw
  ess_beta_iw <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_stan_iw$beta, stan_iter*n_chains, 0)))
  ess_pcor_iw <- mean(sapply(results, function(x) 
    compute_mean_ess(x$fit_stan_iw$rho, stan_iter*n_chains, 0)))
