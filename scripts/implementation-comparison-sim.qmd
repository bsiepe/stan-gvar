...
saveRDS(sim_results, file = here("/output/implementation-sim-results.RDS"))
sim_results <- readRDS(here("output/implementation-sim-results.RDS"))
...
saveRDS(sim_results, file = "~/stan-gvar/output/implementation_sim_results.RDS")
```
...
Organize the data into a longer format for plotting. 
```{r}
sim_results %>% 
  ...
  pivot_longer(cols = contains(c("bias", "mse", "pmad")), 
               names_to = "pm", 
               values_to = "value") %>% 
  # disregard everything with pmad for now
  filter(!str_detect(pm, "pmad")) %>%
  # separate the pm column into the performance measure, matrix, and the method
  separate(pm, into = c("pm", "mat", "method"), sep = "_") %>% 
  ggplot(aes(x = tp, y = value, color = method))+
  geom_point()+
  ggh4x::facet_nested(dgp ~ pm + mat, scales = "free")+
  # theme_compare()+
